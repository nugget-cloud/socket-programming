...
int main()
{
	...
	//ready for listening
	listen(sockfd, 10);
	printf("Connection Established(3 Way Handshake)\n");
	...
	while(1)
	{
		...
		switch(choice)
		{
			case 1: printf("The Array you Entered\n");
                    for(int i=0 ; i<sz ; i++)
						printf("%d ", buffer[i]);
					...
					printf("\nThe Array in Ascending order is sent to the client!\n");
			...
			case 2:// hav to sort it in dsc order
					...
					printf("\nThe Array in Descending order is sent to the client!\n");
					send(afd, buffer, 40, 0);
					break;
